SUBROUTINE write_result_ensight(xEnsight, yEnsight,u,v,omega,press,temp,div,nx,ny,nz,istep,isto,nstep,nstep_fin,t,dt)


   implicit none

   real*8, dimension(nx,ny), intent(in) :: u,v,omega,press,div
   real*8, dimension(-2:nx+3,-2:ny+3) :: temp
   real*8, dimension(3,nx,ny,nz) :: vitesse
   real*8, dimension(1,nx,ny,nz) :: scalar
   integer, parameter :: iskip = 1
   integer, parameter :: nbVar = 5
   integer :: nbImages,ntini
   character(len=80) :: caseName,speedName,omegaName,presName,tempName,divName
   ...
   varType(1) = 2
   write(varName(2),'(A)') 'vort'
   varType(2) = 1
   write(varName(3),'(A)') 'press'
   varType(3) = 1
   write(varName(4),'(A)') 'temp'
   varType(4) = 1
   write(varName(5),'(A)') 'div'
   varType(5) = 1
   
   nbImages = nstep/isto
   istep_rep=istep+nstep_fin
   ...
   if (nstep/isto .eq. 1) then
       ...
       write(omegaName,'(A,I6.6)') 'vort'
       write(presName,'(A,I6.6)') 'press'
       write(tempName,'(A,I6.6)') 'temp'
       write(divName,'(A,I6.6)') 'div'
   else
       write(speedName,'(A,I6.6)') 'vitesse',istep_rep
       write(omegaName,'(A,I6.6)') 'vort',istep_rep
       write(presName,'(A,I6.6)') 'press',istep_rep
       write(tempName,'(A,I6.6)') 'temp',istep_rep	
       write(divName,'(A,I6.6)') 'div',istep_rep
   end if
   call EnsightCase(nbVar,varName,caseName,VarType,nstep_fin,istep,nstep,isto,t,dt)
   ...
   do j=1,ny
      do i=1,nx
        ...
        vitesse(1,i,j,1) = u(i,j)
        vitesse(2,i,j,1) = v(i,j)
        vitesse(3,i,j,1) = 0.
      enddo
   enddo
   call WriteEnsightVar(3,nx,ny,nz,vitesse,speedName,imin,imax,jmin,jmax,kmin,kmax)
   ...
   do j=1,ny
      do i=1,nx   
      scalar(1,i,j,1)=temp(i,j)       
      enddo
   enddo
   call WriteEnsightVar(1,nx,ny,nz,scalar,tempName  ,imin,imax,jmin,jmax,kmin,kmax)

   do j=1,ny
      do i=1,nx   
      scalar(1,i,j,1)=div(i,j)       
      enddo
   enddo
   call WriteEnsightVar(1,nx,ny,nz,scalar,divName  ,imin,imax,jmin,jmax,kmin,kmax)
...
! 
      subroutine EnsightCase(nbVar,VarName,GeoName,VarType,nstep_fin,istep,nstep,isto,t,dt)
      implicit none

      INTEGER,INTENT(IN)::nbVar,nstep,isto,istep,nstep_fin
   ...
   10 format('FORMAT'            ,/ ,'type: ensight gold',//,'GEOMETRY'          ,/ ,'model:	',A         ,//,'VARIABLE')
      ...
      if (nfile.gt.1) then
      	write(FileUnit,45) nfile,nstep_fin+isto,isto
        write(FileUnit,'(E12.5)') (t+dt*j*isto,j=1,nfile)
      endif
